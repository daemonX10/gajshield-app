...
class StaticAnalyzer:
    def __init__(self, yara_rules_path: str = "yara_rules", vt_api_key: str = None):
        self.file_info = {}
        self._init_magic()
        ...
        try:
            ...
            if self.vt_api_key:
                self._timed_execution(
                    lambda: self.file_info.update({'virustotal': self._check_virustotal(self.file_info['hashes']['md5'])}),
                    name='virustotal'
                )
    ...
    def _analyze_ole(self, file_path: str):
        """Analyze OLE document structure and macros"""
        ...
        try:
            ...
            # Check for known suspicious stream names
...
def analyze_files(files, vt_api_key: str = ""):
    """Analyze multiple files and return results"""
    analyzer = StaticAnalyzer(vt_api_key=vt_api_key)
    results = []
...
def create_gradio_interface():
    ...
    iface = gr.Interface(
        fn=analyze_files,
        inputs=[
            gr.File(file_count="multiple", label="Files to Analyze"),
            gr.Textbox(label="VirusTotal API Key (Optional)", placeholder="Enter your VirusTotal API key")
        ],
        outputs=gr.JSON(),
        title="Static File Analyzer",
        description="Upload files for static analysis. Optionally provide a VirusTotal API key for additional malware scanning.",
        examples=[],
